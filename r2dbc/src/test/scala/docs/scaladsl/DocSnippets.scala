  ...
  val done: Future[Done] =
    Source(users)
      .runWith(
        ...
        R2dbc.sink(user => sqlu"INSERT INTO ALPAKKA_SLICK_SCALADSL_TEST_USERS VALUES(${user.id}, ${user.name})")
      )
  //#sink-example
...
object SlickFlowExample extends App {
  ...
  val done: Future[Done] =
    Source(users)
      .via(
        // add an optional first argument to specify the parallelism factor (Int)
        R2dbc.flow(user => sqlu"INSERT INTO ALPAKKA_SLICK_SCALADSL_TEST_USERS VALUES(${user.id}, ${user.name})")
      )
