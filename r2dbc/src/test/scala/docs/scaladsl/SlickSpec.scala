...
class SlickSpec
  ...
  def getAllUsersFromDb: Future[Set[User]] = R2dbc.source(selectAllUsers).runWith(Sink.seq).map(_.toSet)
  def populate() = {
  ...
  "Slick.source(...)" must {
    ...
    "support multiple materializations" in {
      ...
      val source = R2dbc.source(selectAllUsers)
  ...
  "Slick.sink(..)" must {
    ...
    "insert 40 records into a table (parallelism = 4)" in {
      Source(users)
        .runWith(R2dbc.sink(parallelism = 4, insertUser))
        .futureValue
    ...
    "insert 40 records into a table faster using Flow.grouped (n = 10, parallelism = 4)" in {
      Source(users)
        .grouped(10)
        .runWith(
          R2dbc.sink(parallelism = 4, (group: Seq[User]) => group.map(insertUser(_)).reduceLeft(_.andThen(_)))
        )
        .futureValue
